---
title:  "[Git] Git & Github의 동작원리 이해하기 및 주요 명령어 정리"
...
![png](/assets/images/git/git_1.png){: width="70%" height="70%"}   
...
Github은 위에서 설명한 Git의 저장소(변경내용을 담는 공간)를 다른 사람과 공유할 수 있도록 도와주는 웹기반 호스팅 서비스이다. 버전관리 한 내용(git)을 클라우드 서버에 업로드해서 동료들과 개발을 협업한다.  
...
![png](/assets/images/git/git_diag_1.png){: width="70%" height="70%"} 
...
여기서 우리가 잘 알고있는 원격(ex. GitHub)의 개념이 추가된다.  
...
2. **내 작업 폴더와 원격 폴더의 버전차이**가 있을까?  
  - 이젠 혼자 작업하는게 아니므로, 원격과 로컬(내 작업 폴더)간에 버전차이가 생기기 시작한다.  
...
Git/Github의 동작방식은 필요 사항들을 충족시켜주는 큼직한 뼈대를 가지고 있다.  

또한 이러한 내용들은 우리가 잘 알고있는 Git의 용어들로 치환된다.   
...
로컬 컴퓨터의 작업 폴더를 Git Repository로 적용함으로써 프로젝트를 생성하는 방식이다.  
...
기존에 존재하는(또는 Github에서 만든) Remote Repository로 부터 프로젝트를 생성하는 방식이다.  

- Remote Repository를 Local로 복사   
